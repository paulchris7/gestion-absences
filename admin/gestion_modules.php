...
try {
    ...
    if (isset($_GET['delete']) && !empty($_GET['delete'])) {
        ...
        if ($module_to_delete) {
            ...
            if ($count_inscriptions > 0 || $count_seances > 0) {
                ...
                $stmt = $pdo->prepare("DELETE FROM module_responsable WHERE module_id = :module_id");
                $stmt->execute(['module_id' => $id]);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $responsable_ids = $_POST['responsable_ids'] ?? [];
        $id = $_POST['id'] ?? '';
        
        // Validation des champs
        if (empty($code) || empty($nom) || empty($filiere_id) || empty($semestre)) {
                ...
                try {
                    ...
                    if (!empty($responsable_ids)) {
                        foreach ($responsable_ids as $responsable_id) {
                            $stmt = $pdo->prepare("
                                INSERT INTO module_responsable (module_id, responsable_id)
    ...
    if (isset($_GET['edit']) && !empty($_GET['edit'])) {
        ...
        if ($module) {
            ...
            $stmt = $pdo->prepare("
                SELECT responsable_id FROM module_responsable WHERE module_id = :module_id
            ");
            $stmt->execute(['module_id' => $id]);
    ...
    <div class="card-body">
        <form action="gestion_modules.php<?php echo $filiere_filter ? "?filiere=$filiere_filter" : ''; ?>" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="code" name="code" value="<?php echo $module['code']; ?>" required>
                <small>Ex: GI101, RSSP201...</small>
            </div>
            
            <div class="form-group">
                <label for="nom">Nom du module</label>
                <input type="text" class="form-control" id="nom" name="nom" value="<?php echo $module['nom']; ?>" required>
            </div>
                    ...
                    <div class="card-body">
                        <?php foreach ($responsables as $responsable): ?>
                            <div class="form-check">
                                <input type="checkbox" id="responsable_<?php echo $responsable['id']; ?>" name="responsable_ids[]" value="<?php echo $responsable['id']; ?>" 
                                    <?php echo (in_array($responsable['id'], $module['responsables'] ?? [])) ? 'checked' : ''; ?>>
                                <label for="responsable_<?php echo $responsable['id']; ?>">
                                    <?php echo $responsable['prenom'] . ' ' . $responsable['nom']; ?>
                                </label>
                            </div>
                        <?php endforeach; ?>
                    </div>
                ...
                <tbody>
                    <?php foreach ($modules as $m): ?>
                        <tr>
                            ...
                            <td><?php echo $m['filiere_code'] . ' - ' . $m['filiere_nom']; ?></td>
                            <td><?php echo $m['semestre']; ?></td>
                            <td>
                                ...
                                <?php else: ?>
                                    <ul>
                                        <?php foreach ($m['responsables'] as $resp): ?>
                                            <li><?php echo $resp['prenom'] . ' ' . $resp['nom']; ?></li>
                                        <?php endforeach; ?>
                                    </ul>
                                <?php endif; ?>
                            </td>
                            <td class="table-actions">
                                <a href="gestion_modules.php?edit=<?php echo $m['id']; ?><?php echo $filiere_filter ? "&filiere=$filiere_filter" : ''; ?>" class="btn btn-sm btn-warning">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="#" class="btn btn-sm btn-danger delete-btn" data-id="<?php echo $m['id']; ?>" data-name="<?php echo $m['code'] . ' - ' . $m['nom']; ?>">
