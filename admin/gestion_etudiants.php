...
try {
    ...
    $query = "
        SELECT e.*, f.code as filiere_code, f.nom as filiere_nom
        FROM etudiants e
    ...
    <div class="card-body">
        <form action="gestion_etudiants.php" method="get" class="flex gap-10" style="flex-wrap: wrap;">
            ...
            <div class="form-group" style="flex: 2; min-width: 300px;">
                ...
                <input type="text" class="form-control" id="search" name="search" value="<?php echo $search; ?>" placeholder="Nom, prénom, apogée ou email...">
            </div>
                ...
                <tbody>
                    <?php foreach ($etudiants as $etudiant): ?>
                        <tr>
                            ...
                            <td><?php echo $etudiant['filiere_code']; ?></td>
                            <td>
                                <?php if (empty($etudiant['modules'])): ?>
                                    <em>Aucun module</em>
                                <?php else: ?>
                                    <button class="btn btn-sm btn-secondary show-modules" data-etudiant="<?php echo $etudiant['id']; ?>">
                                        Voir les modules (<?php echo count($etudiant['modules']); ?>)
                                    </button>
                                    <div class="modules-list" id="modules-<?php echo $etudiant['id']; ?>" style="display: none;">
                                        <div class="form-group">
                                            <label><strong>Semestre 1</strong></label>
                                            <ul>
                                                <?php foreach ($etudiant['modules'] as $module): ?>
                                                    <?php if ($module['semestre'] === 'S1'): ?>
                                                        <li><?php echo $module['code'] . ' - ' . $module['nom']; ?></li>
                                                    <?php endif; ?>
                                                <?php endforeach; ?>
                                            </ul>
                                        </div>
